...
export const rearrangeItinerary = async (req: Request, res: Response) => {
  try {
    ...
    const placesInfo = currentPlaces.map((p: any, index: number) => ({
      id: p.place._id,
      name: p.place.name,
...
export const autoGenerateTrip = async (req: Request, res: Response) => {
  try {
    ...
    const placesInfo = availablePlaces.map((p: any) => ({
      id: p.placeId || p._id,
      name: p.name,
