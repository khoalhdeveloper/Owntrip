...
exports.InventoryController = {
     ...
     */
    bulkCreateInventory: async (req, res) => {
        try {
            ...
            while (currentDate < end) {
                ...
                const inventory = await roomInventory_model_1.default.findOneAndUpdate({ hotelId, roomTypeId, date }, {
                    hotelId,
                    roomTypeId,
                    date,
                    totalInventory,
    ...
    getInventory: async (req, res) => {
        try {
            ...
            const inventories = await roomInventory_model_1.default.find(query).sort({ date: 1, roomTypeId: 1 });
            // Tính thống kê
            const stats = {
    ...
    updateInventory: async (req, res) => {
        try {
            ...
            if (priceAtDate !== undefined)
                updateData.priceAtDate = priceAtDate;
            if (status !== undefined)
                updateData.status = status;
            const inventory = await roomInventory_model_1.default.findByIdAndUpdate(id, updateData, { new: true });
            if (!inventory) {
    ...
    bulkPriceUpdate: async (req, res) => {
        try {
            ...
            if (fixedPrice) {
                ...
                const inventories = await roomInventory_model_1.default.find(query);
                for (const inv of inventories) {
                    await roomInventory_model_1.default.findByIdAndUpdate(inv._id, { priceAtDate: Math.floor(inv.priceAtDate * priceMultiplier) });
                }
            ...
            const result = await roomInventory_model_1.default.updateMany(query, updateQuery);
            res.status(200).json({
                success: true,
    ...
    deleteInventory: async (req, res) => {
        try {
            ...
            const result = await roomInventory_model_1.default.deleteMany(query);
            res.status(200).json({
                success: true,
    ...
    getDashboard: async (req, res) => {
        try {
            ...
            const inventories = await roomInventory_model_1.default.find({
                hotelId,
