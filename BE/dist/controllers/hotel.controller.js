...
exports.HotelController = {
    ...
    getAllHotels: async (req, res) => {
        try {
            ...
            const hotels = await hotel_model_1.default.find(filter).sort({ createdAt: -1 });
    ...
    getHotelFullPage: async (req, res) => {
        try {
            ...
            const endDate = checkOut ? new Date(checkOut) : tomorrow;
    ...
    createHotel: async (req, res) => {
        try {
            ...
            if (inventorySetup && inventorySetup.length > 0) {
                ...
                await roomInventory_model_1.default.deleteMany({ hotelId: savedHotel.hotelId });
    ...
    getMyHotels: async (req, res) => {
        try {
            ...
            const hotels = await hotel_model_1.default.find({ ownerId: userId });
            res.status(200).json({ success: true, data: hotels });
        }
        catch (error) {
            res.status(500).json({ success: false, message: error.message });
    ...
    assignOwner: async (req, res) => {
        try {
            ...
            const hotel = await hotel_model_1.default.findOneAndUpdate({ hotelId }, { $set: { ownerId } }, { new: true });
            if (!hotel)
                return res.status(404).json({ success: false, message: "Khách sạn không tồn tại" });
            res.status(200).json({
                success: true,
