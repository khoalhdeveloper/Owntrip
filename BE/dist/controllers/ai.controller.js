...
exports.scoreItinerary = exports.autoGenerateTrip = exports.rearrangeItinerary = void 0;
...
const cleanGeminiJson = (responseText) => responseText.replace(/```json/g, "").replace(/```/g, "").trim();
const callGeminiJson = async (prompt, temperature) => {
...
const assertDayOwner = async (dayId, userId) => {
...
const assertAllDaysOwner = async (dayIds, userId) => {
    if (dayIds.length === 0) {
        return true;
    }
    const days = await planDay_model_1.default.find({ _id: { $in: dayIds } });
    if (days.length !== dayIds.length) {
        return false;
    }
    const tripIds = Array.from(new Set(days.map((day) => String(day.tripId))));
    const ownedTrips = await trip_model_1.default.countDocuments({ _id: { $in: tripIds }, userId });
    return ownedTrips === tripIds.length;
};
const buildPlaceInfoFromBody = (currentPlaces) => {
    return currentPlaces.map((item, index) => {
...
const loadPlaceInfoForDay = async (dayId) => {
    const places = await planPlace_model_1.default.find({ dayId }).sort({ order: 1, createdAt: 1 });
...
const validateOrderedPlaceIds = (orderedPlaceIds, placesInfo) => {
...
const buildFallbackChanges = (orderedPlaceIds, placesInfo) => {
    const placeByAnyId = new Map();
...
`;
        ...
        const orderedPlaceIds = Array.isArray(parsedData.orderedPlaceIds)
            ? parsedData.orderedPlaceIds.map((id) => String(id))
            : [];
...
const autoGenerateTrip = async (req, res) => {
    try {
        ...
        const dayIds = days.map((day) => day.dayId).filter(Boolean).map((dayId) => String(dayId));
...
const scoreItinerary = async (req, res) => {
    try {
...
exports.scoreItinerary = scoreItinerary;
